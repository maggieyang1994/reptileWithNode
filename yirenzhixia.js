...
// const downloadItem = (url, path, cb) => {
...
// const downloadAll = (chapterList, cb) => {
//     async.eachLimit(
//         chapterList,
//         10,
//         (chapter, chapterCb) => {
//             console.log(`正在下载 ${chapter.name} ...`)
//             mkdirp(`./data/${chapter.name}`, function (err) {
//                 if (err) return console.err(err)
//                 async.eachLimit(
//                     chapter.imgList,
//                     10,
//                     (item, imgCb) => {
//                         const fileName = item.match(/[\d]+\.jpg/)[0]
//                         console.log(`正在下载 ${chapter.name} - ${fileName}`)
//                         downloadItem(item, `./data/${chapter.name}/${fileName}`, () => imgCb(null))
//                     },
//                     (err, result) => {
//                         chapterCb()
//                     }
...
// }
                    ...
                    // pool
                    //     .query(`insert into yirenzhixia.directory(chapter_id, chapter_name, first_img_url) values(${item.sid}, '${item.name}', '${firstImgPath}');`)
                    //     .then(res => {
                    //         computeAllImg([{ ...item, firstImgPath }])
                    //         console.log(`保存 ${item.name} 成功。`)
                    //         cb(null, { ...item, firstImgPath })
                    //     })

                    // pool
        // .query(`insert into yirenzhixia.ImageList(chapter_id, image_url) values(${chapter.sid}, '${imagUrl}');`)
        // .then(res => {
        //     if (res.serverStatus === 2) {
        //         // console.log(`保存 ${chapter.name} 成功。`)
        //     }
        // })
    ...
    // pool
    //     .query('select * from directory where chapter_id > 381;')
    //     .then(res => {
    //         let chapterList = res.map(x => ({
    //             name: x.chapter_name,
    //             sid: x.chapter_id,
    //             firstImgPath: x.first_img_url
    //         }))
    //         chapterList = computeAllImg(chapterList)
    //         downloadAll(chapterList, () => {
    //             console.log('Done!')
    //         })
    //     })
    // downloadAll(chapterList)